...
def CanRepair():
    """Return whether the current base has a ship upgrade/repair shop."""
    norepair = {
...
def ShipPurchase(shipname):
    ...
    VS.StopAllSounds()
    if (VS.getPlayer().getName() == shipname or
        VS.getPlayer().getName() == shipname+".blank" or
        VS.getPlayer().getName() + ".blank" == shipname
        ):
...
def CargoValue(un):
    """Value the cargo of the supplied unit (typically the player's ship)."""
    numcarg = un.numCargo()
    tot = 0
    for i in range(numcarg):
        c = un.GetCargoIndex(i)
        if c.GetCategory().find("upgrades") == 0:
            tot += c.GetPrice() * c.GetQuantity() * USED_VALUE
    return tot;
