...
OLDVERSIONTEXT="privateer100" # The hidden folder in the data dir, with the m3us
NEWVERSIONTEXT="privateer100" # What you want the hidden folder to be called

DATAMODULE="priv" # The folder with the data in it
...
if [ $DESTRUCTIVE = 0 -a $CLEANREPOSITORY = 0 ] ; then
    echo "NOT DESTRUCTIVE"
fi
...
if [ $SETUPARG = "prepare" ] || [ $SETUPARG = "organise" ] || [ $SETUPARG = "package" ] || [ $SETUPARG = "release" ]; then
  SCRIPTFAILED=0
fi
...
#    find $CURPWD/../$DATAMODULE -name *.xmesh -exec /bin/rm -r {} \; #Removes xmesh files
  fi
...
# Move the vegastrike music
    ...
    cp -r $CURPWD/../$MUSICMODULE/*  $MUSICDIR/music/
    cp -rf $CURPWD/../$MUSICMODULE/.[^.]* $MUSICDIR
    mv -f $MUSICDIR/music/sounds $MUSICDIR/sounds
  fi
