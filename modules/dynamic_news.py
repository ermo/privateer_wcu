...
def fixShipName(stri): # removes suffixes from ship names, looks silly otherwise
    try:
        strin = string.replace(stri, ".blank","")
        strin = string.replace(strin,".Blank","")
        strin = string.replace(strin,".stock","")
        strin = string.replace(strin,".Stock","")
        strin = string.replace(strin,"decommissioned","")
        strin = string.replace(strin,"Decommissioned","")
        strin = string.replace(strin,"deprecated","")
        strin = string.replace(strin,"Deprecated","")
        strin = string.replace(strin,".template","")
        strin = string.replace(strin,".Template","")
        strin = string.replace(strin,"Mk"," Mk")
        strin = string.replace(strin,"  Mk"," Mk")
        return strin
    except:
        return stri
...
class DynamicNewsData:
    ...
    def getFactionData(self, faction, variable):
        ...
        if variable in self.faction_dict["alltags"]:
            try:
                if faction in self.faction_dict:
                    return self.faction_dict[faction][variable]
                elif faction.lower() in self.faction_dict:
                    return self.faction_dict[faction.lower()][variable]
                else:
