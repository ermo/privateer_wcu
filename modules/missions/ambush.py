...
debug.debug("importing ambush.py")
...
class ambush(directions_mission.directions_mission):
    ...
    def Execute(self):
        #debug.debug("Ambush Execute: you = VS.getPlayerX(self.cp=%s) = %s" % (self.cp, VS.getPlayerX(self.cp)))
        directions_mission.directions_mission.Execute(self)
        if self.terminated == 1:
            return
        you = VS.getPlayerX(self.cp)
        if you.isNull():
            debug.info("Ambush: self.you is null?!")
            return
