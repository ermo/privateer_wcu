...
class directions_mission (Director.Mission):
    def dir_privateSetupPlayer(self, cp):
        ...
        debug.info("jumps: %s" % (str(self.jumps)))
        self.arrived = 0
        self.wasnull = 0
    ...
    def __init__(self, savevar, jumps=(), destination=''):
        ...
        debug.info("Directions Mission: Starting")
        global isambushrunning
        self.var = savevar
        self.savedCargo = self.getCargo(VS.getPlayer())
        debug.debug("self.savedCargo: %s" % (str(self.savedCargo)))
        if (self.var, self.savedCargo) in isambushrunning:
            #VS.terminateMission(0)
            debug.info("Directions Mission: Stopping: directions already running! (before mission restore)")
        isambushrunning[(self.var, self.savedCargo)] = True
        self.jumps = jumps
        debug.debug("Directions Mission: self.cp = %s" %(VS.getCurrentPlayer()))
    ...
    def Execute(self):
        #debug.debug("Directions Mission Execute: you = VS.getPlayerX(self.cp=%s) = %s" % (self.cp, VS.getPlayerX(self.cp)))
