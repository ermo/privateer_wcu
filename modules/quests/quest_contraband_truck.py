...
class quest_contraband_truck (quest.quest):
    ...
    def mission_fail(self):
        ...
        debug.info("mission terminating")
        debug.info("mission terminated")
        self.quest_exit = 1
    ...
    def start_destruction(self):
        if self.repeat_end2 == 2 and not self.cargo_container.isNull():
            ...
            debug.info("ended msgs")
            self.jumpout = 1
            self.repeat_end2 = 0
            debug.info("ended start_destruction")

    def end_destruction(self):
        #debug.info("testing timer")
        if (self.timer1 + 12) <= VS.GetGameTime():
            debug.info("playing sounds")
        ...
        if (self.playa):
            ...
            if self.truck_pirate.getMinDis(self.playa.Position()) < 200:
                ...
                if self.repeat_end1 == 1:
                    ...
                    debug.info("My target is...")
                    global truck_exit
                    truck_exit = 1
                    debug.info(truck_exit)
                    self.repeat_more = 0
                    self.repeat_less = 0
                    self.repeat_end1 = 2
